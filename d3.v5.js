...
var frame = 0, // is an animation frame pending?
    ...
    setFrame = function(f) { setTimeout(f, 0); };
  ...
  function force(alpha) {
    for (var k = 0, n = links.length; k < iterations; ++k) {
      for (var i = 0, link, source, target, x, y, l, b; i < n; ++i) {
        ...
        // console.log(source.x,source.y,target.x,target.y,source.vx,source.vy,target.vx,target.vy)
        l = Math.sqrt(x * x + y * y);
        ...
        source.vx += x * (b = 1 - b);
        source.vy += y * b;
        // console.log(source.x,source.y,target.x,target.y,source.vx,source.vy,target.vx,target.vy,x,y,l,alpha, strengths[i],bias[i]);
      }
    }
  }
...
function simulation(nodes) {
  var simulation,
      alpha = 1,
      alphaMin = 0.01,
      alphaDecay = 1 - Math.pow(alphaMin, 1 / 400),
      alphaTarget = 0,
      velocityDecay = 0.6,
      forces = map$1(),
      stepper = timer(step),
...
function x$2(x) {
  ...
  function force(alpha) {
    for (var i = 0, n = nodes.length, node; i < n; ++i) {
      ...
      // console.log(xz[i],strengths[i], alpha);
    }
  }
